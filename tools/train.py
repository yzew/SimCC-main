...
# CUDA_VISIBLE_DEVICES=3 python tools/train.py --cfg experiments/coco/hrnet/sa_simdr/gaugcn51.yaml
...
# CUDA_VISIBLE_DEVICES=0,1,2 python tools/test.py --cfg experiments/coco/hrnet/sa_simdr/gaugcn6.yaml TEST.MODEL_FILE /dataset/wh/wh_code/SimCC-main/output2/coco2/gaugcn6/gaugcn6/model_best.pth TEST.USE_GT_BBOX False
# gaugcn5  bs=100?
# CUDA_VISIBLE_DEVICES=0,1,2,3 python tools/train.py --cfg experiments/coco/hrnet/sa_simdr/gaugcn5.yaml
...
# CUDA_VISIBLE_DEVICES=0,1,2 python tools/train.py --cfg experiments/coco/hrnet/heatmap/w32_seg_point.yaml
# CUDA_VISIBLE_DEVICES=0,1,2 python tools/test.py --cfg experiments/coco/hrnet/heatmap/w32_seg_point.yaml TEST.MODEL_FILE /dataset/wh/wh_code/SimCC-main/output/coco3/segpoint/w32_seg_point/model_best.pth
# CUDA_VISIBLE_DEVICES=0,1,2 python tools/test.py --cfg experiments/coco/hrnet/heatmap/w32_256x192_adam_lr1e-3.yaml TEST.MODEL_FILE /dataset/wh/wh_code/HRNet-Human-Pose-Estimation-master/output/coco/pose_hrnet/w32_256x192_adam_lr1e-3/model_best.pth
# CUDA_VISIBLE_DEVICES=0,1,2 python tools/test.py --cfg experiments/coco/hrnet/heatmap/w32_seg_point3.yaml TEST.MODEL_FILE /dataset/wh/wh_code/SimCC-main/output/coco3/segpoint3/w32_seg_point3/model_best.pth

# python tools/train.py --cfg experiments/coco/hrnet/heatmap/w32_seg_point31.yaml
# CUDA_VISIBLE_DEVICES=0,1,2 python tools/test.py --cfg experiments/coco/hrnet/heatmap/w32_seg_point31.yaml

# CUDA_VISIBLE_DEVICES=0,1,2 python tools/train.py --cfg experiments/coco/hrnet/sa_simdr/gaugcn51.yaml
# CUDA_VISIBLE_DEVICES=3 python tools/train.py --cfg experiments/coco/hrnet/heatmap/w32_seg_point31.yaml

# CUDA_VISIBLE_DEVICES=3 python tools/train.py --cfg experiments/coco/hrnet/heatmap/mscam.yaml
...
from core.loss import JointsMSELoss, NMTCritierion, NMTNORMCritierion, KLDiscretLoss, JointEdgeSegLoss,CrossEntropy, BCE, BoneLoss, BoneLoss2
from core.function import train_heatmap, train_heatmap_, train_heatmap2, train_simdr, train_sa_simdr,train_sa_simdr2,train_sa_simdr3,train_sa_simdr4
from core.function import validate_heatmap, validate_heatmap_, validate_heatmap2, validate_simdr, validate_sa_simdr,validate_sa_simdr2,validate_sa_simdr3,validate_sa_simdr4
...
def main():
    ...
    logger.info("Total number of trainable parameters: %d" % trainable_pytorch_total_params)
    ...
    if cfg.LOSS.TYPE == 'JointsMSELoss':
        ...
        criterion2 = BCE().cuda()
        #criterion2 = JointEdgeSegLoss(classes=1,
                                        #  ignore_index=0,
                                        #  edge_weight=1, seg_weight=1,
                                        #  att_weight=1).cuda()
        ...
        #     use_target_weight=cfg.LOSS.USE_TARGET_WEIGHT
        # ).cuda()
    elif cfg.LOSS.TYPE == 'KLDiscretLoss2':
        criterion = KLDiscretLoss().cuda()        
        criterion2 = BoneLoss(joint_parents=[0, 0, 1, 2, 0, 4, 5, 0, 7, 8, 9, 8, 11, 12, 8, 14, 15]).cuda()
    elif cfg.LOSS.TYPE == 'KLDiscretLoss3':
        criterion = KLDiscretLoss().cuda()        
        criterion2 = BoneLoss2().cuda()
    else:
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        if cfg.MODEL.COORD_REPRESENTATION == 'simdr':
            ...
            perf_indicator = validate_sa_simdr(
                cfg, valid_loader, valid_dataset, model, criterion,
                final_output_dir, tb_log_dir, writer_dict) 
            # perf_indicator = validate_sa_simdr2(
            #     cfg, valid_loader, valid_dataset, model, criterion, criterion2,
            #     final_output_dir, tb_log_dir, writer_dict) 
        elif cfg.MODEL.COORD_REPRESENTATION == 'sa-simdr2':
            # train_sa_simdr2(cfg, train_loader, model, criterion, criterion2, optimizer, epoch,
            #   final_output_dir, tb_log_dir, writer_dict)
            train_sa_simdr3(cfg, train_loader, model, criterion,criterion2, optimizer, epoch,
              final_output_dir, tb_log_dir, writer_dict)
            
            perf_indicator = validate_sa_simdr3(
                cfg, valid_loader, valid_dataset, model, criterion,
                final_output_dir, tb_log_dir, writer_dict)
        elif cfg.MODEL.COORD_REPRESENTATION == 'sa-simdr3':
            # train_sa_simdr2(cfg, train_loader, model, criterion, criterion2, optimizer, epoch,
            #   final_output_dir, tb_log_dir, writer_dict)
            train_sa_simdr4(cfg, train_loader, model, criterion,criterion2, optimizer, epoch,
              final_output_dir, tb_log_dir, writer_dict)
            
            perf_indicator = validate_sa_simdr4(
                cfg, valid_loader, valid_dataset, model, criterion,
                final_output_dir, tb_log_dir, writer_dict)  
        elif cfg.MODEL.COORD_REPRESENTATION == 'heatmap':
            train_heatmap_(cfg, train_loader, model, criterion, optimizer, epoch,
                final_output_dir, tb_log_dir, writer_dict)

            perf_indicator = validate_heatmap_(
                cfg, valid_loader, valid_dataset, model, criterion,
                final_output_dir, tb_log_dir, writer_dict
            )
            ...
            #     final_output_dir, tb_log_dir, writer_dict
            # )
        elif cfg.MODEL.COORD_REPRESENTATION == 'heatmap2':
            train_heatmap2(cfg, train_loader, model, criterion, criterion2, optimizer, epoch,
                final_output_dir, tb_log_dir, writer_dict)

            perf_indicator = validate_heatmap2(
                cfg, valid_loader, valid_dataset, model, criterion,
                final_output_dir, tb_log_dir, writer_dict
            )
