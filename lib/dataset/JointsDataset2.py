...
class JointsDataset2(Dataset):
    def __init__(self, cfg, root, image_set, is_train, transform=None, coord_representation='heatmap', simdr_split_ratio=1):
        ...
        assert self.coord_representation in ['simdr', 'sa-simdr', 'sa-simdr2', 'heatmap'], 'only simdr or sa-simdr or heamtap supported'
    ...
    def __getitem__(self, idx):
        ...
        if self.coord_representation == 'simdr':
            ...
            target_weight = torch.from_numpy(target_weight)
            return input, target_x, target_y, target,target_weight, target_weight_2, meta, meta_2
        elif self.coord_representation == 'sa-simdr2':
            target_weight_2 = target_weight
            meta_2 = meta
            target_x, target_y, target_weight = self.generate_sa_simdr(joints, joints_vis)
            target_x = torch.from_numpy(target_x)
    ...
    def generate_target(self, joints, joints_vis):
        ...
        target_weight = np.ones((self.num_joints, 1), dtype=np.float32)
        target_weight[:, 0] = joints_vis[:, 0]
        self.heatmap_size[1] = 64
        self.heatmap_size[0] = 48
...
class JointsDataset3(Dataset):
    def __init__(self, cfg, root, image_set, is_train, transform=None, coord_representation='heatmap2', simdr_split_ratio=1):
        self.num_joints = 0
        self.pixel_std = 200
        self.flip_pairs = []
        self.parent_ids = []
        ...
        self.simdr_split_ratio = simdr_split_ratio
        self.loss = cfg.LOSS.TYPE
        assert self.coord_representation in ['simdr', 'sa-simdr', 'heatmap', 'heatmap2'], 'only simdr or sa-simdr or heamtap supported'
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            input = self.transform(input)
            #mask = self.transform(mask) # normalize
        ...
        elif self.coord_representation == 'heatmap2':
            return input, target, target_weight, meta,mask
