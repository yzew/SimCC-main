...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.gcn128 = SemGCN(self.adj, 512, 3072, 128)
        self.gau = GatedAttentionUnit(
            hidden_size=512,#512
    ...
    def forward(self, x):
        ...
        if self.coord_representation == 'heatmap':
            ...
            x = self.mlpp(x)  # -> B, K, hidden

            # x_gcn # 80,64,64,48 # low feature
            x_gcn = self.CtoK(x_gcn) # trans to B 17 64 48  # no loss
            x_gcn_1 = rearrange(x_gcn, 'b c h w -> b c (h w)') # trans to B 17 3072
