...
# class SemGraphConv1(nn.Module):
#     """
#     Semantic graph convolution layer
#     """

#     def __init__(self, in_features, out_features, adj, bias=True):
#         super(SemGraphConv1, self).__init__()
#         self.in_features = in_features
#         self.out_features = out_features
#         self.position_embedding = PositionEmbeddingLearned(out_features)
...
#         pe = self.position_embedding(input).transpose(1,2).contiguous()
#         print(pe.shape)
#         #print(self.W.shape)
#         self.W.data = self.W.data + pe
...
#         M = torch.eye(adj.size(0), dtype=torch.float).to(input.device) # 生成对角线全1，其余部分全0的二维数组
...
class _GraphConv(nn.Module):
    def __init__(self, adj, input_dim, output_dim, p_dropout=None):
        ...
        #self.gconv = SemGraphConv1(input_dim, output_dim, adj)
...
class PositionEmbeddingLearned(nn.Module):
    ...
    def __init__(self,out_channel):
        super().__init__()
        self.out=out_channel//2
        self.row_embed = nn.Embedding(128, self.out) # 第一个维度必须大于64.第二个维度表示输出维度/2，这里9的话输出18个
        self.col_embed = nn.Embedding(128, self.out)
...
class PositionEmbeddingLearned2(nn.Module):
    """
    Absolute pos embedding, learned.
    """
    def __init__(self,out_channel):
...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        #self.aspp = _AtrousSpatialPyramidPoolingModule(224, 45, output_stride=8)
        
        #self.pos_embed = nn.Parameter(torch.zeros(1, 1, 64, 48))

        self.position_embedding = PositionEmbeddingLearned2(256)
    ...
    def forward(self, x):
        ...
        if self.coord_representation == 'heatmap':
            ...
            # 生成256个64*48的位置嵌入，并拉伸转置作为一个权重
            pe = self.position_embedding(x_).transpose(1, 2) # [B, 256, 3072] -> [B, 3072, 256]
            x_e = torch.bmm(x, pe)
            #print(x_e.shape)
            


            matrix = self.mlpp(x)  # -> B, K, hidden(256)
            #matrix = torch.softmax(matrix + x_e, dim = -1)
            matrix = self.sigmoid(matrix + x_e)
            x = self.mlp_v(x) * matrix

            #x = torch.mul(self.mlpp(x), x_e)
            #x = self.mlpp(x) + x_e
