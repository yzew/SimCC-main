...
class PRM(nn.Module):
    ...
    def __init__(self, output_chl_num, efficient=False):
        ...
        #self.avg_pool = nn.AdaptiveAvgPool2d((1,1)) # B C 1 1
...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.prm32 = PRM(output_chl_num=32)
        self.prm64 = PRM(output_chl_num=64)
        self.prm128 = PRM(output_chl_num=128)
    ...
    def forward(self, x, mask):
        ...
        y_list[0] = self.prm32(torch.cat((self.conv31(y_list[0]),x1),1))
        y_list[1] = self.prm64(torch.cat((self.conv63(y_list[1]),x2),1))
        y_list[2] = self.prm128(torch.cat((self.conv127(y_list[2]),x3),1))
