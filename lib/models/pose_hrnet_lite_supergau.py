...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.gau768 = GatedAttentionUnit(
            hidden_size=768,
        ...
        self.gau192 = GatedAttentionUnit(
            hidden_size=192,
    ...
    def forward(self, x):
        ...
        for i in range(self.stage3_cfg['NUM_BRANCHES']):
            if self.transition2[i] is not None:
                ...
                # meigaihao
                ww = self.transition2[i](y_list[-1]).shape[3]
                hh = self.transition2[i](y_list[-1]).shape[2]
                x = rearrange(self.transition2[i](y_list[-1]), 'b c h w -> b c (h w)') # 32, 17, 3072 # import einops
                pred_x_gau = self.gau192(x, None, None, False)
                ...
                outputs_x = torch.unsqueeze(outputs_x, 3)
                outputs_x = rearrange(outputs_x, 'b c (h w1) w -> b c h (w w1)', w1=ww) # 32, 17, 3072 # import einops

                x_list.append(outputs_x)
                ...
                ww = y_list[i].shape[3]
                hh = y_list[i].shape[2]
                x = rearrange(y_list[i], 'b c h w -> b c (h w)') # 32, 17, 3072 # import einops
                if (ww * hh == 3072) :
                    pred_x_gau = self.gau(x, None, None, False)
                else :
                    pred_x_gau = self.gau768(x, None, None, False)
