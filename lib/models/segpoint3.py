...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.conv32 = nn.Conv2d(33, 32, kernel_size = 1, stride = 1, padding = 0)
        self.conv64 = nn.Conv2d(65, 64, kernel_size = 1, stride = 1, padding = 0)
        self.conv128 = nn.Conv2d(129, 128, kernel_size = 1, stride = 1, padding = 0)
        ...
        #self.scaleoffset = ScaleOffset(1, offset=self.use_bias)
    ...
    def forward(self, x, mask):
        ...
        # mask = self.scaleoffset(mask) # mask * W
        x1 = F.interpolate(mask, scale_factor=0.25)
        x2 = F.interpolate(mask, scale_factor=0.125)
        x3 = F.interpolate(mask, scale_factor=0.0625)
        y_list[0] = self.conv32(torch.cat((y_list[0],x1),1))
        y_list[1] = self.conv64(torch.cat((y_list[1],x2),1))
        y_list[2] = self.conv128(torch.cat((y_list[2],x3),1))
