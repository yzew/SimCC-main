...
# class SemGCN(nn.Module):
#     #     def __init__(self, adj, hid_dim, coords_dim=(2, 3), num_layers=4, nodes_group=None, p_dropout=None):
#     def __init__(self, adj, hid_dim, coords_dim=(512, 384), num_layers=2, nodes_group=None, p_dropout=None):
#         super(SemGCN, self).__init__()
...
#     #     def __init__(self, adj, hid_dim, coords_dim=(2, 3), num_layers=4, nodes_group=None, p_dropout=None):
#     def __init__(self, adj, hid_dim, coords_dim=(512, 512), num_layers=2, nodes_group=None, p_dropout=None):
...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.bsample = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True) #input 4D: B C W H
        self.nsample = nn.Upsample(scale_factor=4, mode='nearest')
        # stem net
    ...
    def forward(self, x):
        #print(x.shape) # torch.Size([1, 3, 64, 48])
        x = self.bsample(x)
        #print(x.shape) # torch.Size([1, 3, 256, 192])
        ...
        if self.coord_representation == 'heatmap':
            ...
            pred_x = self.avg_pool(pred_x)
            pred_y = self.avg_pool(pred_y)
