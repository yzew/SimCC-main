...
class BasicBlock2(nn.Module):
    ...
    def forward(self, x):
        ...
        inputs = rearrange(x, 'b c h w -> b c (h w)')
        size = x.size()

        x = F.interpolate(input=x, size=(16, 12), mode='bilinear', align_corners=False)
        x = rearrange(x, 'b c h w -> b c (h w)')

        out = self.gau(x)
        pred_x_gau = list(out)
        pred_x_gau[0] = pred_x_gau[0].reshape(size[0], size[1], 16, 12)
        pred_x_gau[0] = F.upsample(pred_x_gau[0], size=(size[2],size[3]), mode="bilinear", align_corners=True)
        pred_x_gau[0] = rearrange(pred_x_gau[0], 'b c h w -> b c (h w)')

        o_x = F.dropout(pred_x_gau[0], p=0.1, training=self.training)
        o_x = self.norm(inputs + o_x )
        outputs_x = o_x # add attentions if we output them
        ...
        outputs_x = outputs_x.reshape(size[0], size[1], size[2], size[3])
...
class GatedAttentionUnit(nn.Module):
    ...
    def __init__(
            ...
            stride = 1,
            downsample = None,
            intermediate_size=384,# 1536
            attention_key_size=96,
            activation="swish",
            use_bias=False,
        ...
        #self.position_embeddings = FixedAbsolutePositionEmbedding(max_position_embeddings=512,hidden_size=hidden_size)
    ...
    def forward(
        ...
        #sinusoidal_pos = self.position_embeddings(x)
        # 投影变换
...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.transition3_ = self._make_transition_layer(
            pre_stage_channels, num_channels)
        self.stage4_, pre_stage_channels = self._make_stage(
            self.stage4_cfg, num_channels)
        
        self.stage5_cfg = extra['STAGE5']
        num_channels = self.stage5_cfg['NUM_CHANNELS']
        block = blocks_dict2[self.stage5_cfg['BLOCK']]
        num_channels = [
            num_channels[i] for i in range(len(num_channels))
        ]
        self.transition4 = self._make_transition_layer(
            pre_stage_channels, num_channels)
        self.stage5, pre_stage_channels = self._make_stage_(
    ...
    def _make_stage_(self, layer_config, num_inchannels,
                    multi_scale_output=True):
        num_modules = layer_config['NUM_MODULES']
        num_branches = layer_config['NUM_BRANCHES']
        num_blocks = layer_config['NUM_BLOCKS']
        num_channels = layer_config['NUM_CHANNELS']
        block = blocks_dict2[layer_config['BLOCK']]
        fuse_method = layer_config['FUSE_METHOD']
    ...
    def forward(self, x):
        ...
        for i in range(self.stage4_cfg['NUM_BRANCHES']):
            if self.transition3_[i] is not None:
                x_list.append(self.transition3_[i](y_list[-1]))
            else:
                x_list.append(y_list[i])
        y_list = self.stage4_(x_list)
