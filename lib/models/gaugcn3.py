...
class GatedAttentionUnit(nn.Module):
    ...
    def forward(
        ...
        if x_gcn  is not None:
            # q = x_gcn
            q = self.q_scaleoffset(x_gcn)
        else:
            q = self.q_scaleoffset(z)
        # print(z.shape) # B K 128
...
# tb_logger = SummaryWriter('run/example')
...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.transs = nn.Linear(3072, 2)
        self.transto128 = nn.Linear(3072, 128)
        ...
        self.gcn384 = SemGCN(self.adj, 512, 512, 384)
        self.gcn512 = SemGCN(self.adj, 512, 512, 512)
        self.gcn128 = SemGCN(self.adj, 512, 192, 128)
        self.gau = GatedAttentionUnit(
            hidden_size=512,#512
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        # print(x.shape) # 80,64,128,96
        x = self.conv2(x)
        x = self.bn2(x)
        x = self.relu(x)
        # x_gcn = x
        # print(x.shape) # 80,64,64,48
        x = self.layer1(x)
        # print(x.shape) # 80,256,64,48
        ...
        x_list = []
        for i in range(self.stage3_cfg['NUM_BRANCHES']): # 3,0 1 2
        ...
        x_gcn = x_list[2]
        # print(x_gcn.shape) # 80,128,16,12
        ...
        if self.coord_representation == 'heatmap':
            ...
            # tmp = self.transs(x)
            # x = self.gcn(tmp)
            # x = self.transto3072(x)
            x = self.mlpp(x)  # -> B, K, 512

            # x_gcn # 80,128,16,12 # low feature
            x_gcn = self.CtoK(x_gcn) # 128 to 17
            
            # # viusal
            # for i in range (0,17):
            #     tb_logger.add_image('channels', # nrow:nummber of images displayed per line
            #     make_grid(x_show[0].detach().cpu().unsqueeze(dim=1)[i], nrow=1, padding=2, normalize=True, pad_value=0),global_step=i)
            
            # x_show = F.interpolate(x_gcn, scale_factor=10, mode = 'bilinear')
            # grid = make_grid(x_show[0].detach().cpu().unsqueeze(dim=1), nrow=5, padding=2, normalize=True, pad_value=0)
            ...
            # grid = make_grid(x_final[0].detach().cpu().unsqueeze(dim=1), nrow=5, padding=2, normalize=True, pad_value=0)
            # image_grid = Image.fromarray(grid.mul(255).permute(1,2,0).byte().numpy())
            # image_grid.save("run/example2/" + str(np.random.uniform(1,10000)) + ".jpg")


            x_gcn_1 = rearrange(x_gcn, 'b c h w -> b c (h w)') # trans to B 17 192
            x_gcn_1 = self.gcn128(x_gcn_1) # trans to B 17 128
            # print(x_gcn.shape)

            # # self-gau
            # x_gcn_2 = self.gau2(x_gcn_1, None, None, None, False)
            # o_x_1 = F.dropout(x_gcn_2[0], p=0.1, training=self.training)
            # o_x_1 = self.norm(x_gcn_1 + o_x_1)
            # outputs_x_1 = (o_x_1,) + x_gcn_2[1:]  # add attentions if we output them
            # # an di 0 weidu pinjie tezhengtu 
            # outputs_x_1 = torch.cat([fm for fm in outputs_x_1], dim = 0)
            # # outputs_x = torch.as_tensor([item.cpu().detach().numpy() for item in outputs_x], device = torch.device('cuda'))
            # outputs_x_1 = torch.squeeze(outputs_x_1, 0)
            # # print(outputs_x_1.shape) 80 17 128

            # # x_gcn as query
            # # pred_x_gau = self.gau(x, x_gcn, None, None, False)
            # pred_x_gau = self.gau(x, outputs_x_1, None, None, False)

            pred_x_gau = self.gau(x, x_gcn_1, None, None, False)
